...
func TestOneComplexCycle(t *testing.T) {
	...
	index := lo.GroupBy(results, func(row *analysis.Row) string {
...
func assertNotPartOfCycle(t *testing.T, results []*analysis.Row, component ...string) {
	index := lo.GroupBy(results, func(row *analysis.Row) string {
		return row.Data["component"].(string)
	})
...
func assertHasCycle(t *testing.T, results []*analysis.Row, expectedCycle []string) {
	grouped := lo.GroupBy(results, func(row *analysis.Row) string {
		return strconv.Itoa(row.Data["cycle_nr"].(int))
	})

	for _, theCycle := range grouped {
		components := lo.Map(theCycle, func(row *analysis.Row, _ int) string {
			return row.Data["component"].(string)
		})
	...
	for _, theCycle := range grouped {
		formattedGroups = append(formattedGroups, lo.Map(theCycle, func(row *analysis.Row, _ int) string {
			return row.Data["component"].(string)
		}))
	}
